...
- Allow managing instance profiles named `nodes-karpenter-*` in the Crossplane policy
...
- Add `additional_policies` and `additional_policies_arns` support to the crossplane module
...
- Add Route53 permissions for `crossplane-fn-irsa`
...
- Add permissions to access EKS to admin and readonly roles
...
- Limit some of the `GiantSwarmAdmin` role permissions
...
- Specify looser version requirements for the AWS provider.
...
- Add support for removing IAM permissions from the capa controller role when EKS support is not required.
- Add support for removing IAM permissions from the capa controller role when resolver rules support is not required.
- Instead of using `iam:*` for capa-iam-operator, specify precisely what it needs.
...
- Add `iam:TagInstanceProfile`, `iam:UntagInstanceProfile`, `iam:UntagPolicy`, `iam:UntagRole` permissions to admin role.
...
[Unreleased]: https://github.com/giantswarm/giantswarm-aws-account-prerequisites/compare/v8.3.0...HEAD
